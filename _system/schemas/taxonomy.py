...
# ---------------------------------------------------------------------------
# Repo classification (scalar — repos stay 1:1 with a single collection)
# ---------------------------------------------------------------------------


class RepoClassificationLLMOutput(BaseModel):
    """Raw structured output from the classify_repo LLM call.

    Repos remain scalar: exactly one (domain, collection) pair. The
    paper-side multi-collection split deliberately doesn't apply here —
    a repo is a single artifact, not a paper that genuinely contributes
    to multiple categories. ``collection_index`` mirrors the historic
    paper shape: ``-1`` to propose a new collection, otherwise the index
    into the chosen domain's collection list.
    """
...
class RepoClassificationOutput(BaseModel):
    """Resolved repo classification, post index→name lookup."""
