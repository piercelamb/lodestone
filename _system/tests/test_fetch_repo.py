...
under ``tmp_path``. ``code_files`` and ``readmes_fts`` are keyed by
``repo_id`` now; ``fetch_repo`` is dispatched by ``repo_slug``.
"""
...
def _seed_repo(
    ...
    *,
    ...
    cur = conn.execute(
        ...
        ) VALUES (?, ?, 'github.com', 'owner', 'repo', ?, ?, ?, ?, ?)
        """,
        (
            repo_slug, url, paper_id,
            "2024-01-02T00:00:00+00:00", status.value, domain, collection,
        ),
    )
...
def test_clone_failure_sets_failed_repo(conn, monkeypatch):
    ...
    fr.fetch_repo(conn=conn, repo_slug="gh-owner-repo")
    status = conn.execute(
        "SELECT status FROM repos WHERE id = ?", (rid,)
    ).fetchone()[0]
    assert status == RepoStatus.FAILED_REPO.value
...
def test_walks_and_inserts_text_files(conn, monkeypatch):
    ...
    rows = conn.execute(
        ...
        " WHERE repo_id = ? ORDER BY path", (rid,)
    ).fetchall()
...
def test_skips_binary_extensions(conn, monkeypatch, name):
    ...
    paths = {r[0] for r in conn.execute(
        "SELECT path FROM code_files WHERE repo_id = ?", (rid,)
    ).fetchall()}
...
def test_skips_files_with_high_replacement_char_density(conn, monkeypatch):
    rid = _seed_repo(conn)
    payload = bytes(range(0x80, 0xff)) * 4
...
def test_flattens_ipynb_keeps_code_and_markdown_cells_drops_outputs(
    ...
    row = conn.execute(
        ...
        " WHERE repo_id = ? AND path = ?", (rid, "notebook.ipynb")
    ).fetchone()
...
def test_commit_sha_persisted_to_repos(conn, monkeypatch):
    rid = _seed_repo(conn)
    monkeypatch.setattr(
        fr, "_clone_repo",
    ...
    sha, fetched = conn.execute(
        "SELECT commit_sha, fetched_at FROM repos WHERE id = ?", (rid,)
    ).fetchone()
...
def test_top_level_readme_md_indexed_into_readmes_fts(conn, monkeypatch):
    ...
    fr.fetch_repo(conn=conn, repo_slug="gh-owner-repo")
    rows = conn.execute(
        "SELECT path, repo_slug, content FROM readmes_fts WHERE repo_id = ?",
        (rid,),
    ).fetchall()
    assert len(rows) == 1
    path, slug, content = rows[0]
    assert path == "README.md"
    assert slug == "gh-owner-repo"
...
def test_readme_priority_md_over_rst_over_txt_over_extensionless(
    ...
    fr.fetch_repo(conn=conn, repo_slug="gh-owner-repo")
    row = conn.execute(
        "SELECT path FROM readmes_fts WHERE repo_id = ?", (rid,)
    ).fetchone()
...
def test_no_readme_in_repo_no_row_in_readmes_fts(conn, monkeypatch):
    ...
    # has_readme flag is recorded on repos.
    has_readme = conn.execute(
        "SELECT has_readme FROM repos WHERE id = ?", (rid,)
    ).fetchone()[0]
    assert has_readme == 0
...
def test_force_replaces_readme_row(conn, monkeypatch):
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?)",
        (rid, "gh-owner-repo", "rag", "README.md", "STALE README"),
    )
    monkeypatch.setattr(fr, "_clone_repo", _FakeClone({
    ...
    fr.fetch_repo(conn=conn, repo_slug="gh-owner-repo")
    rows = conn.execute(
        "SELECT content FROM readmes_fts WHERE repo_id = ?", (rid,)
    ).fetchall()
    assert len(rows) == 1
